...
test('鉴权、IMEI、W0/W1入库与去重', async () => {
  ...
  await bind('P03', '862323089930703');
...
test('缺测值不写成0且旧读数不覆盖latest', async () => {
  ...
  const latestBefore = database.prepare('SELECT collected_at FROM latest_readings WHERE plot_id = (SELECT id FROM plots WHERE plot_code = ?)').get('P03').collected_at;
  await ingest(w1Payload('862323089930703', base - 3600));
  const latestAfter = database.prepare('SELECT collected_at FROM latest_readings WHERE plot_id = (SELECT id FROM plots WHERE plot_code = ?)').get('P03').collected_at;
...
test('CSV含BOM和标准列，清理测试数据保留配置与绑定', async () => {
  ...
  assert.equal(database.prepare('SELECT COUNT(*) AS count FROM plot_bindings WHERE active = 1').get().count, 2);
});
