...
- 默认部署为共享IP访问模式：页面仍为 `http://106.14.8.100`，写入独立Rice Nginx配置 `/etc/nginx/conf.d/rice.conf`。
- 共享IP模式必须保留LanSense的 `/api/device-ingest/` → `127.0.0.1:3001`；Rice设备只允许使用 `/rice-api/device-ingest/` → `127.0.0.1:3201`。禁止两个项目复用同一上报路径。
...
LANSENSE_BACKEND_PORT=3001
LANSENSE_PUBLIC_INGEST_PREFIX=/api/device-ingest/
RICE_PUBLIC_INGEST_PREFIX=/rice-api/device-ingest/
```
...
脚本依次以较低CPU/IO优先级执行依赖安装、自动测试和前端构建，再进行SQLite迁移、Rice独立PM2进程重载、Rice独立Nginx配置、`nginx -t`、reload和 `/ready` 验证。共享IP模式会先检查LanSense的3001 `/ready`，未就绪则拒绝安装网关；LanSense上报路径和Rice上报路径分别使用明确的location。如设置 `RICE_SKIP_NGINX=true`，会删除Rice遗留的Nginx配置后再切换到3201直连，避免旧IP路由继续生效。任一步失败都会停止，不会启停或改写LanSense服务。
...
sudo nginx -t
curl -i http://106.14.8.100/api/device-ingest/d100l2
curl -i http://106.14.8.100/rice-api/device-ingest/d100l2
```
